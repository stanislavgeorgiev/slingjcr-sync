...
var Monitor = new Class({
	...
	options: {
		...
		directory: '',
		output: '',
		match: '**/*.+(js|css|jsp|less)', //'**/*.less',
		ignore: '',
		extension: '.css',
		optimization: 0,
		compress: false,
		interval: 250,
		nofollow: false,  // should be true but this causes failure
		master: false
...
//            var repoUrl = 'http://localhost:8080/apps/';
            var repoUrl = 'http://localhost:4502/';
            var fileUrl = repoUrl + (""+filePath).replace(/\\/g,"/");

            console.log('uploading file '+filePath+ ' to '+ fileUrl);

//            var command =  "\\cygwin\\bin\\curl -u admin:admin -T apps/g5/components/flights/templates/searchTypeRoundTripLanding/searchTypeRoundTripLanding.jsp http://localhost:4502/apps/g5/components/flights/templates/searchTypeRoundTripLanding/searchTypeRoundTripLanding.jsp"
            var command =  "\\cygwin\\bin\\curl -u admin:admin -T"
